...
=======
>>>>>>> 974e6588f58db056f3d76672636cfb9c01e9caf2
...
We have learnt in previous sessions how to construct (=train) statistical models from sets of examples (training sets) that can formulate predictions for previously unseen data (the prediction set).

The construction of the training sets has not been covered in depth in this course, but it is one of the most critical stages of statistical learning. The training set will often be a sample of the population or universe to which our models will be applied. Amongst other, we should aim at collecting training sets that

1. represent the population prevalences, 
...
An optimal (in the sense of supervised learning) representation space is one that includes all features that convey information needed for the inference at stake, and minimizes redundancy.  
...
Now, why should we want to use an optimal representation space? There are five main reasons for this:
...
While feature extraction creates new features by transforming and combining  the original features, feature selection only returns a subset of these original features. 
...
After rescaling, our data set occupies a unit hypercube in `r n` dimensions. 

Now, let us ask ourselves how many examples do we need to ensure that the closest example (in the to-be-determined sense) is not further than one particular example: one example that differs from our observed spectrum by a 10% of the total range [0,1] in each flux. This is already very far! So one would naïvely expect that this loose requirement leads to a need for few training examples. 
...
These are all theoretical definitions. In practice, because the training set is but a sample from the population, these black/white definitions expand to form a full grey-scale. And actually, due to the noise, there would not be irrelevant features. Therefore, more pragmatic approaches are needed.
...
In this case, Spearman's rank correlation coefficient measures correlation, not between the variables themselved, but between their ranks. A practical recipe is:
...
In the XOR example, there was redundancy between the two sets $\{X_4\}$ and $\{X_2,X_3\}$. We can build two decision trees that  
...
We have seen that an *a priori* optimal feature set maximizes relevance and minimizes redundancy. This takes to the problem of selecting feature subsets rather than ranking features, which is what we have done so far.
...
### Searching in the space of feature subsets: 
...
I will skip the algebraic details (although for the particle physics community here they would trivial). In brief, we will be diagonalizing the covariance matrix, and the axes of the new basis (the eigenvectors) will be the axes of the ellipsoid. The eigenvalue represents the length of the vector or, in other words, the percentage of variance explained by that direction.
...
$$
\mathbb{x}_i = a_{i1}\mathbb{s}_1+a{i2}\mathbb{s}_2+...+a{iN}\mathbb{s}_N
$$
...
pch=0.6+(StPa[,2]+1)/6
plot3d(lle$X[,1],lle$Y[,2],lle$Y[,3],col=col,size=pch,ty="s")
...
dists <- dist(df2)
clusters <- diffuse(dists, eps.val = epsilonCompute(dists), # The Gaussian kernel variance
...
col <- SetupPalette(df[,1062])
pch=0.6+(StPa[,2]+1)/6
#plot3d(lle$X[,1],lle$X[,2],lle$X[,3],col=col,size=pch,ty="s")
